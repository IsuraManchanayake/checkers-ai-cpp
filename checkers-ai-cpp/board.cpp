...
namespace checkers_AI {
    ...
    void board::_execute_move(const move& move) {
                    ...
                    for (int j = 2; j < board_max_dim; j++) {
                        ...
                        if (_occupiable(tpos_) &&
                            ...
                            for (; _occupiable(tpos_); tpos_ += dir) {
                                move move_(mover, start, tpos_, captured);
            ...
            if (mover->is_queen) {
                for (const vec& dir : vec::dirs) {
                    for (int j = 1; j < board_max_dim; j++) {
                        ...
                        if (_occupiable(to_)) {
                            moves_.push_back(move(mover, to_));
                        }
                        else {
                            break;
                        }
                ...
                if (_occupiable(to1_)) {
                    moves_.push_back(move(mover, to1_));
                }
                if (_occupiable(to2_)) {
                    moves_.push_back(move(mover, to2_));
                }
            }
    ...
    std::vector<piece*> board::list_blance_pieces(const move& last_move) {
        std::vector<piece*> blance_pieces_;
        if (!last_move.is_empty && last_move.move_kind == move::move_kind_type::move) {
    ...
    std::vector<move> board::list_all_moves(const move& last_move, piece* mover) {
        ...
        for (piece* blanced_piece : blance_pieces_) {
            ...
            for (auto& move_ : moves_after_blance_) {
    ...
    std::vector<move> board::list_all_moves(const move& last_move, piece::color_type color) {
        std::vector<piece*> blance_pieces_ = list_blance_pieces(last_move);
        std::vector<move> moves_ = _list_all_raw_moves(color);
        for (piece* blanced_piece : blance_pieces_) {
            _board[blanced_piece->pos.y][blanced_piece->pos.x] = piece::make_empty();
            std::vector<move> moves_after_blance = _list_all_raw_moves(color);
            for (auto& move_ : moves_after_blance) {
                move_.blance_piece = blanced_piece;
            }
